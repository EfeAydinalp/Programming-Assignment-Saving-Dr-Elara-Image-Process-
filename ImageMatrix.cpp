...
ImageMatrix ImageMatrix::operator+(const ImageMatrix &other) const {
    ImageMatrix returnMatrix = ImageMatrix(height,width);
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            returnMatrix.data[i][j] = data[i][j] + other.get_data(i,j);
        }
    }
    return returnMatrix;
}
...
ImageMatrix ImageMatrix::operator*(const double &scalar) const {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            returnMatrix.data[i][j] = data[i][j] * scalar;
        }
    }
    return returnMatrix;
}
...
void ImageMatrix::setData(int i,int j, double value) {
